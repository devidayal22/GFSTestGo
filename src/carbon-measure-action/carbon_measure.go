...
func main() {
		...
		if ts.Resource == "Microsoft.Compute/virtualMachines" {
			for _, S := range Sizes {
				for _, D := range S.Details {
					...
					fmt.Println("D.Location  " + D.Location)
					fmt.Println("D.Resource  " + ts.Resource)
					fmt.Println(S.Size)
	...
	githubNoticeMessage("grams_carbon_equivalent_per_kwh   " + fmt.Sprint(averageKwh))
	githubNoticeMessage("grams_emitted_over_24h   " + fmt.Sprint(Totalco2perkwh))
}
